...
export const fetchWeatherForecastByCity = createAsyncThunk<
  IWeatherForecastEntry,
  string,
  { rejectValue: string }
>("weather/fetchForecastByCity", async (city: string, { rejectWithValue }) => {
  try {
    if (!API_KEY) {
      throw new Error("OpenWeatherMap API Key is not defined");
    }
    ...
    const requestUrl = `${BASE_URL}/forecast`;
    const params = {
      q: city,
      appid: API_KEY,
    };

    const response = await $axios.get<IWeatherForecastEntry>(requestUrl, {
      params: params,
    });
    return response.data;
  } catch (e) {
